  ...
  template: `
    <div class="ui-dialog" role="dialog" aria-modal="true" [attr.aria-labelledby]="titleId" cdkTrapFocus *transloco="let t">
      <header class="ui-dialog__header">
        <h2 [id]="titleId">{{ t('dashboard.components.newFolder.title') }}</h2>
      ...
      <div class="ui-dialog__body">
        <label class="nf__label" [attr.for]="fieldId">{{ t('dashboard.components.newFolder.label') }}</label>
        <input
          #field
          uiInput
          type="text"
        ...
        @if (error(); as message) {
          ...
          <p class="nf__hint">{{ t('dashboard.components.newFolder.parentHint', { parent: data.parentName }) }}</p>
        }
      </div>
      ...
      <footer class="ui-dialog__footer">
        ...
        <button type="button" uiButton variant="primary" [loading]="saving()" [disabled]="!valid() || saving()" (click)="submit()">
          {{ t('dashboard.components.newFolder.create') }}
        </button>
      </footer>
    </div>
  `,
    ...
    try {
      ...
      this.error.set(
        e instanceof ApiError && e.code === 'NAME_TAKEN'
          ? this.transloco.translate('dashboard.components.newFolder.nameTaken')
          : e instanceof Error && e.message
            ? e.message
            : this.transloco.translate('dashboard.components.newFolder.createFailed'),
      );
    } finally {
