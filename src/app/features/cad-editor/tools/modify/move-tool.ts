...
import { snapshotEntity, moveEntityInPlace } from '../geometry-utils';
import { beginDragPreview, endDragPreview, drawTransformGhost, commitEntityTransforms } from '../drag-preview';

/** Minimal move: select first, hit 'M', click base point, click destination. */
export class MoveTool implements ITool {
  ...
  onMouseDown(wx: number, wy: number, sx: number, sy: number, e: MouseEvent): void {
    ...
    this.applyDelta(wx - this.basePoint.x, wy - this.basePoint.y);
  }
  ...
  private applyDelta(dx: number, dy: number): boolean {
    ...
    // Entities were never mutated during the drag (ghost-only preview), so
    // they're still at their pre-drag state — apply once and record as a
    // single atomic undo step.
    commitEntityTransforms(this.snapshots, (e: any) => moveEntityInPlace(e, dx, dy), this.cmds, this.vm);
    this.cleanup();
    this.dyn.clearEdits();
    this.tools.setTool('select');
    return true;
  ...
  onMouseMove(wx: number, wy: number): void {
    ...
    // Ghost-only preview: do not touch the real entities here. drawPreview()
    // renders the moving ghost; the canvas redraws on this markDirty.
    this.vm.markDirty();
  }
  ...
  drawPreview(ctx: CanvasRenderingContext2D): void {
    ...
    drawTransformGhost(ctx, this.vm, this.doc, this.targets, { kind: 'move', dx, dy });
