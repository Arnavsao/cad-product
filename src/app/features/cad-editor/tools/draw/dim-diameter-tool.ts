...
import { DiameterDimensionEntity } from '../../core/models/entity-extended.model';
...
export class DimDiameterTool implements ITool {
  ...
  onMouseDown(wx: number, wy: number): void {
    ...
    if (this.phase === 'dimline') {
      const dim = new DiameterDimensionEntity(this.center!, this.arcPoint!);
      ...
      if (this.targetEntity?.id !== undefined) {
        dim.anchorCircle = { entityId: this.targetEntity.id, snapIndex: 0 };
      }
